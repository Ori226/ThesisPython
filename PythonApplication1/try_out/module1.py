...
covariances = np.random.random_sample((2, 4)) + 1

y[0:N/4] = 0
y[N/4:N/2] = 1
y[N/2:(3*N)/4] = 2
y[(3*N)/4:] = 3
#lets modify it to be the xor problem:
#a = 1,1 or -1,-1
#b = 1,-1 or -1,1



new_means = np.array([[1, -1, 1, -1], [1, -1, -1, 1]])*3
...
X = np.vstack([np.random.randn(N)*covariances[0, y] + new_means[0, y],
               np.random.randn(N)*covariances[1, y] + new_means[1, y]])
# Plot the data
...
plt.scatter(X[0, :], X[1, :], c=y, lw=.6, s=12, cmap=plt.cm.cool)
plt.axis([-6, 6, -6, 6])
plt.show()
...
mlp = MyMLP.MyMLPModule(W_init, b_init, activations)
...
train = theano.function([mlp_input, mlp_target], cost,
                        updates=my_functions.gradient_updates_momentum(cost, mlp.params, learning_rate, momentum))
...
max_iteration = 5000
X = my_functions.np_gpu_arr(X)
y = my_functions.np_gpu_arr(y)
...
while iteration < max_iteration:
        ...
        #            lw=.8, s=40, cmap=plt.cm.cool, vmin=0, vmax=1)
        #plt.axis([-6, 6, -6, 6])
